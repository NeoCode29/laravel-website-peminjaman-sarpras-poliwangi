...
@section('title', 'Assign Approval Sarana')
@section('subtitle', 'Kelola approver untuk sarana yang memerlukan persetujuan')

@section('content')
<section class="detail-page approval-sarana-page">
    <div class="card user-detail-card">
        <div class="card-main">
            ...
            @if(request('sarana_id'))
                <div class="alert alert-info">
                    ...
                    Menampilkan approvers untuk sarana: <strong>{{ $saranas->where('id', request('sarana_id'))->first()->name ?? 'Tidak ditemukan' }}</strong>
                    <a href="{{ route('approval-assignment.sarana.index') }}" class="btn btn-sm btn-secondary">
            ...
            <div class="detail-block">
                ...
                <tbody>
                    @forelse($saranaApprovers as $index => $approver)
                        <tr>
                            <td>{{ $saranaApprovers->firstItem() + $index }}</td>
                            <td>
                                <div class="sarana-info">
                                    <div class="sarana-image">
                                        @if($approver->sarana->image_url)
                                            <img src="{{ Storage::url($approver->sarana->image_url) }}" alt="{{ $approver->sarana->name }}">
                                        @else
                                            <div class="sarana-placeholder">
                                                <i class="fas fa-box"></i>
                                            </div>
                                        @endif
                                    </div>
                                    <div class="sarana-details">
                                        <strong>{{ $approver->sarana->name }}</strong>
                                        <small>{{ $approver->sarana->kategori->name ?? 'Tidak ada kategori' }}</small>
                                    </div>
                                </div>
                            </td>
                            <td>
                    ...
                    @empty
                        <tr>
                            <td colspan="7" class="empty-state">
                                ...
                                <p>Belum ada approver sarana yang ditetapkan</p>
                            </td>
                        </tr>
                    @endforelse
                </tbody>
        ...
        <div class="pagination-info">
            <p>Menampilkan {{ $saranaApprovers->firstItem() }} sampai {{ $saranaApprovers->lastItem() }} dari {{ $saranaApprovers->total() }} data</p>
            <div class="pagination">
                {{ $saranaApprovers->links() }}
            </div>
        </div>
    </div>
</div>
...
<div id="addModal" class="modal">
    <div class="modal-content">
        <div class="modal-header">
            <h3>
                <i class="fas fa-plus"></i>
                Tambah Sarana Approver
            </h3>
            <button type="button" class="close" onclick="closeModal('addModal')">&times;</button>
        </div>
        <form action="{{ route('approval-assignment.sarana.store') }}" method="POST" onsubmit="return validateForm(this)">
            @csrf
            <div class="modal-body">
                <div class="form-group">
                    <label for="sarana_id">Sarana <span class="required">*</span></label>
                    <select name="sarana_id" id="sarana_id" required>
                        <option value="">Pilih Sarana</option>
                        @foreach($saranas as $sarana)
                            <option value="{{ $sarana->id }}" {{ (old('sarana_id') == $sarana->id || request('sarana_id') == $sarana->id) ? 'selected' : '' }}>
                                {{ $sarana->name }} - {{ $sarana->kategori->name ?? 'Tidak ada kategori' }}
                            </option>
                        @endforeach
                    </select>
                    @error('sarana_id')
...
<div id="bulkModal" class="modal">
    <div class="modal-content">
        <div class="modal-header">
            <h3>
                <i class="fas fa-layer-group"></i>
                Bulk Assign Sarana Approvers
            </h3>
            <button type="button" class="close" onclick="closeModal('bulkModal')">&times;</button>
        </div>
        <form action="{{ route('approval-assignment.sarana.bulk-assign') }}" method="POST" onsubmit="return validateBulkForm(this)">
            @csrf
            <div class="modal-body">
                <div class="form-group">
                    <label for="bulk_sarana_ids">Pilih Sarana <span class="required">*</span></label>
                    <select name="sarana_ids[]" id="bulk_sarana_ids" multiple required>
                        @foreach($saranas as $sarana)
                    ...
                    <small>Gunakan Ctrl+Click untuk memilih multiple sarana</small>
                </div>
...
function validateBulkForm(form) {
    const saranaIds = form.querySelectorAll('#bulk_sarana_ids option:checked');
    ...
    if (saranaIds.length === 0) {
        alert('Pilih minimal satu sarana');
        return false;
    }
