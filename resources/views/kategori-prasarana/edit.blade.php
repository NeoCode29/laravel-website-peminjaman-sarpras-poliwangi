...
<section class="detail-page prasarana-detail-page">
    <div class="card user-detail-card">
        <div class="card-main">
            <form action="{{ route('kategori-prasarana.update', $kategoriPrasarana->id) }}" method="POST" class="kategori-form" id="kategoriPrasaranaEditForm">
                @csrf
                @method('PUT')
                        ...
                        <div class="detail-block form-grid">
                            ...
                            <div class="form-group">
                                ...
                                <textarea id="description"
                                          ...
                                          rows="4">{{ old('description', $kategoriPrasarana->description) }}</textarea>
                                @error('description')
                                    <div class="form-error">{{ $message }}</div>
                                @enderror
                            </div>
                    ...
                    <div class="form-section">
                        ...
                        <div class="detail-block preview-block">
                            <div class="preview-item">
                                ...
                                <span class="preview-value" id="previewName">{{ $kategoriPrasarana->name }}</span>
                            </div>
                            <div class="preview-item">
                                <span class="preview-label">Deskripsi</span>
                                <span class="preview-value" id="previewDescription">{{ $kategoriPrasarana->description ?: '-' }}</span>
                            </div>
                            <div class="preview-item">
                            ...
                            <div class="preview-item">
                                <span class="preview-label">Jumlah Prasarana</span>
                                <span class="preview-value">
                                    <span class="status-badge status-approved">{{ $kategoriPrasarana->prasarana_count ?? 0 }}</span>
                                </span>
                            </div>
                        </div>
                    </div>
...
document.addEventListener('DOMContentLoaded', function() {
    ...
    // Form validation
    const form = document.getElementById('kategoriPrasaranaEditForm');
    form.addEventListener('submit', function(e) {
        ...
        const originalName = '{{ $kategoriPrasarana->name }}';
        if (name === originalName) {
            const originalDescription = '{{ $kategoriPrasarana->description }}';
            if (descriptionInput.value.trim() === originalDescription) {
                e.preventDefault();
    ...
    function showAlert(message, type) {
        ...
        alert.innerHTML = `
            <i class="fas fa-${type === 'error' ? 'exclamation-circle' : type === 'warning' ? 'exclamation-triangle' : 'check-circle'} alert-icon"></i>
            <div>
                <strong>${type === 'error' ? 'Error!' : type === 'warning' ? 'Peringatan!' : 'Berhasil!'}</strong> ${message}
            </div>
        `;
